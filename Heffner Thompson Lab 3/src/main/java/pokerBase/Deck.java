...
public class Deck {
	...
	public Deck(int jokers) {
		//Create an ArrayList of Cards, add each card
		this();
		ArrayList<Card> MakingDeck = cards;

		for (int i = 0; i<jokers; i++) {
			MakingDeck.add(new Card(eSuit.JOKER,eRank.JOKER,53+i));
		}
		//Set the instance variable
		cards = MakingDeck;
		ShuffleCards();
	...
	public Deck(int jokers, ArrayList<Card> wildCards) {
		//Create an ArrayList of Cards, add each card
		this(jokers);
		ArrayList<Card> MakingDeck = cards;
		
		for (Card wild : wildCards) {
			for (Card card : MakingDeck) {
				if (wild == card) {
					card.setWild();
				}
			}
		}
		this.cards = MakingDeck;
		ShuffleCards();
	}
